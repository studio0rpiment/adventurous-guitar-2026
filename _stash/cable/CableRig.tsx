...
import { EXIT_LOCAL, HANG_TOP, JACK_AXIS, ROPE } from "./constants";
...
export function CableRig() {
  ...
  const pts = useMemo(() => createHangingRope(HANG_TOP), []);
  const anchorA = useRef(HANG_TOP.clone());
  const anchorB = useRef(HANG_TOP.clone());
